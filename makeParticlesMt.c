...
void seekParticles(TH1D* H1D[][2], ParticleCoor particle) {
...
void createHistograms(TString aEventDir, TString aOutDir, Int_t aEventFiles, TString inputList) {
    ...
    TString YTitle = "dN/dm_{T} (GeV)^{-1}";
    TString XTitles[_N_PARTICLES_] = {"m_{T p} (GeV)","m_{T K} (GeV)","m_{T #pi} (GeV)"};
    ...
    TH1D* H1D[_N_PARTICLES_][_N_FIGURES_];

    for(int i = 0; i < _N_PARTICLES_; i++) {
        for(int j = 0; j < _N_FIGURES_; j++) {
            H1D[i][j] = new TH1D(Form("%s%s",histNames[i].Data(),figuresNames[j].Data()),Form("%s%s",histNames[i].Data(),figuresNames[j].Data()),XBins,XMin,XMax[i]);
            H1D[i][j]->GetXaxis()->SetTitle(XTitles[i].Data());
...
int makeParticlesMt(TString inputlist = "", TString outfile = "", Long64_t nDesEvents = -1, Int_t maxFiles = -1) {
