...
#define _N_FIGURES_ 7 //6 different types of figures to draw
...
void createHistograms(TString aEventDir, TString aOutDir, Int_t aEventFiles, TString inputList) {
    ...
    TString histNames[_N_FIGURES_] = {"MHist","MPtHist","PtYHist", "YHist","PtMHist","MtMMidYHist","MtMFullYHist"};
    Int_t XBins[_N_FIGURES_] = {1000, 1000, 1000, 1000,1000,1000,1000};
    Float_t XMin[_N_FIGURES_] = {1, 1, 0, -2.2, 1,1,1};
	Float_t XMax[_N_FIGURES_] = {1.6, 1.6, 1.5, 2.2, 1.5,3,3};
    TString YTitles[_N_FIGURES_] = {"dN/dM (GeV/c^{2})^{-1}","dN/dM (GeV/c^{2})^{-1}","d^{2}N/dp_{T}dy (GeV/c)^{-1}", "dN/dy", "d^{2}N/dp_{T}dM","dN/dm_{T}dy (GeV/c^{2})^{-1}","dN/dm_{T} (GeV/c^{2})^{-1}"};
	TString XTitles[_N_FIGURES_] = {"M_{#pi^{+} p} (GeV/c^{2})","M_{#pi^{-} p} (GeV/c^{2})","p_{T} (GeV/c)","rapidity","p_{T} (GeV/c)","m_{T} (GeV/c^{2})","m_{T} (GeV/c^{2})"};
    ...
    for (int i = 0; i < _N_PAIRS_; i++ ){ //for every pair
		for (int j = 0; j < _N_FIGURES_ ; j++) { //for every figure
			...
			else if(j == 4 || j == 5 || j == 6) { //if it is m ranges( pt and mt)
				...
				for (auto lorentz : pairsArr[j])  {
					...
					*/
					if(indM >= 0) {	
						...
						H1D[j][6][indM]->Fill(lorentz.Mt(),1.0/TMath::Power(lorentz.Mt(),1.5)); //full one

						if(lorentz.Rapidity() > 0.69 && lorentz.Rapidity() < 0.79)
							H1D[j][5][indM]->Fill(lorentz.Mt(),1.0/TMath::Power(lorentz.Mt(),2)); //mid rap
...
int makeHistTermMt(TString inputlist = "", TString outfile = "", Long64_t nDesEvents = -1, Int_t maxFiles = -1) {
