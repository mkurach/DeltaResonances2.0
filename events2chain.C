...
// ##########################################################################
  ...
  TChain* tChainParts = new TChain(_PARTICLES_TREE_); //chain na czastki
  TChain* tChainEvent = new TChain(_EVENTS_TREE_);  //chain na eventy

  tChainParts->SetBranchAddress(_PARTICLE_BRANCH_, aParticle); //jaki branch bierze, gdzie wyrzuca entries
  tChainEvent->SetBranchAddress(_EVENTS_BRANCH_,   &tStructEvents); //to samo ale dla eventow
  
  for(Int_t i=0; i<aEventFiles; i++) { //po kazdym pliku
    char Buff[200];
