...
TF1* getBreitWigner(ParticleType *aPartType) {
  static TF1* fun = new TF1("fun",breitWigner,0.0,3.0,3); //mass from 0-3, 3 parameters
  fun->SetParameter(0,1); 
  fun->SetParameter(1,aPartType->GetGamma());
  fun->SetParameter(2,aPartType->GetMass());

  fun->SetParameter(0,1./fun->Integral(0.0,3.0));
...
double Model::CalcMassDistr(ParticleType *aPartType, double &statWeight) {
    ...
    else if ( Abs(pdg) == 12112 || Abs(pdg) == 1214 || Abs(pdg) == 22112 || Abs(pdg) == 32214 || Abs(pdg) == 2122 || Abs(pdg) == 32212 || Abs(pdg) == 2216 || Abs(pdg) == 12216|| Abs(pdg) == 31114 || Abs(pdg) == 1112 ) {
      //N*0(1440),N*0(1520),N*0(1535),Delta0(1600),Delta(1620),N*0(1650),N*0(1675), N*0(1680), Delta++(1600), Delta++(1620)
        TF1 *breitDistr = getBreitWigner(aPartType);
      Double_t mMin = aPartType->GetMass() - 2.0*aPartType->GetGamma();
      Double_t mMax = aPartType->GetMass() + 12.0*aPartType->GetGamma();
      
      double m = breitDistr->GetRandom(mMin,mMax);
      statWeight = 1; //never used ??
      return m;
    }
    else {
	statWeight = 1;
